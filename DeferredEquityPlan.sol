...
contract DeferredEquityPlan {
    ...
    // Setting total shares and annual distribution
    uint total_shares = 1000;
    uint annual_distribution = 250;

    uint start_time = now; // permanently store the time this contract was initialized

    // Setting `unlock_time` to be 365 days from now
    ...
    function distribute() public {
        ...
        // Adding "require" statements to enforce that:
        ...
        // Calculating shares distributed by using the function (now - start_time) / 365 days * the annual distribution
        distributed_shares = ((now - start_time) / 365 days * annual_distribution);

        // Making sure employee does not cash out until after 5+ years
    ...
    // Reverting any Ether sent to the contract directly
